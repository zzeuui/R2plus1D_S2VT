...
def sample_clips_metafunc(clip_num, in_frame_number, stride):
    def sample_clips(video_path):
        ...
        indices = list(range(in_frame_number, int(frame_count - in_frame_number - 1), stride))
        ...
        for index in indices:
            clip_list.append(frames[index - in_frame_number: index + in_frame_number])

        clip_list = np.array(clip_list)

        clip_count = clip_list.shape[0]

        if clip_count >= clip_num:
            frame_indices = np.linspace(0, clip_count, num=clip_num, endpoint=False).astype(int)
            clip_list = clip_list[frame_indices]
            clip_list = np.array(clip_list)

        elif clip_count != 0:
            clip_list = np.pad(clip_list, ((0, clip_num - clip_count), (0, 0), (0, 0), (0, 0), (0, 0)), 'constant')
